...
ts_hypertable_create_internal(
	...
	ArrayType *data_node_arr, bool distributed_is_null, bool distributed, bool generic_ht)
{
	...
	else
	{
		...
		if (NULL != closed_dim_info)
			ts_hypertable_check_partitioning(ht, closed_dim_info->dimension_id);
	}

	retval = create_hypertable_datum(fcinfo, ht, created, generic_ht);
	ts_cache_release(hcache);
 ...
 */
static Datum
ts_hypertable_create_time_specific(PG_FUNCTION_ARGS, bool is_dist_call)
{
...
{
	return ts_hypertable_create_time_specific(fcinfo, false);
}

Datum
ts_hypertable_distributed_create(PG_FUNCTION_ARGS)
{
	return ts_hypertable_create_time_specific(fcinfo, true);
}
...
ts_hypertable_assign_chunk_data_nodes(const Hypertable *ht, const Hypercube *cube)
{
	...
	if (NULL != closed_dim && NULL != closed_dim->dimension_partitions)
	{
		const DimensionSlice *slice =
			ts_hypercube_get_slice_by_dimension_id(cube, closed_dim->fd.id);
